...
def main():
    ...
    cursor.execute("SELECT id, capa, path FROM filmes_saga WHERE capa IS NOT NULL")
    filmes_saga = cursor.fetchall()
    print(f"\n🎬 Processando {len(filmes_saga)} filmes de saga...")
    
    for row in filmes_saga:
        id_filme = row['id']
        capa_origem = row['capa'].replace('Midias', 'Mideas') if row['capa'] else None
        
        if capa_origem and os.path.exists(capa_origem):
        ...
        # Copiar sprite (busca na pasta do vídeo)
        if row['path']:
            pasta = os.path.dirname(row['path'].replace('Midias', 'Mideas'))
